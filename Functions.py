...
class appFunctions():
    ...
    def displayOrders(self):
        query = "SELECT c.name AS customer_name, c.customerID, p.name AS package_name, o.orderID, p.price, o.orderDate, IFNULL((SELECT MAX(pays.date) FROM pays WHERE pays.customerID = c.customerID AND pays.packID = o.packID), NULL) AS payment_date, o.dlvryDate FROM customer c JOIN orders o ON c.customerID = o.customerID JOIN package p ON o.packID = p.packID GROUP BY o.orderID;"
        mycursor.execute(query)
        rows = mycursor.fetchall()
    ...
    def deleteOrder(self):
        key = int(self.ui.deleteOrderNamePack.text())
        

        order_query = "DELETE FROM orders WHERE orderID = %s;"
        pay_query = "DELETE FROM pays WHERE payID = %s;"

        try:
            mycursor.execute(order_query, (key,))
            mycursor.execute(pay_query, (key,))
            mydb.commit()
    ...
    def updateOrderDel(self):
        ordID = int(self.ui.updatePackIDinOrders.text())
        
        pay_query = "UPDATE orders SET dlvryDate = %s WHERE orderID = %s"
        curr = date.today()
        values = (curr, ordID)
    ...
    def updateOrderPay(self):
        payID = int(self.ui.updatePackIDinOrders.text())
        
        pay_query = "UPDATE pays SET date = %s WHERE payID = %s"
        curr = date.today()
        values = (curr, payID)
    ...
    def updateOrderPayReset(self):
        ...
        pay_query = "UPDATE pays SET date = NULL WHERE payID = %s"

        try:
            mycursor.execute(pay_query, (payID,))
            mydb.commit()
            QMessageBox.information(self, "Success", "Updated successfully.")
        except Error as e:
            print(e)
    ...
    def updateOrderOrdReset(self):
        ...
        pay_query = "UPDATE orders SET dlvryDate = NULL WHERE orderID = %s"

        try:
            mycursor.execute(pay_query, (ordID,))
            mydb.commit()
            QMessageBox.information(self, "Success", "Updated successfully.")
        except Error as e:
            print(e)
